...
class Attendance:
    ...
    def __init__(self,root):
        ...
        # first header image  
        img=Image.open(r"E:\final Project\Images_GUI\banner.jpg")
        ...
        studentId_entry = ttk.Entry(left_frame,textvariable=self.var_roll,width=15,font=("verdana",12,"bold"))
        ...
        #create table 
        self.attendanceReport_left = ttk.Treeview(table_frame,column=("Roll_No","Name","Department","Time","Date","Attend"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.attendanceReport_left.heading("Department",text="Dep-name")
        ...
        #create table 
        self.attendanceReport = ttk.Treeview(table_frame,column=("Roll_No","Name","Department","Time","Date","Attend"),xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)
        ...
        self.attendanceReport.heading("Department",text="Std-ID")
    ...
    def update_data(self):
        if self.var_roll.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update attendance_data set std_name=%s,dep_name=%s,std_time=%s,std_date=%s,std_attendance=%s where std_roll_no=%s",( 
                    #self.var_roll.get(),
                    self.var_name.get(),
                    self.var_dep.get(),
                    self.var_time.get(),
    ...
    def delete_data(self):
        if self.var_roll.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = mysql.connector.connect(username='root', password='',host='localhost',database='face_recognizer',port=3306)
                    mycursor = conn.cursor() 
                    sql="delete from attendance_data where std_roll_no=%s"
    ...
    def action(self):
        if self.var_roll.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                conn = mysql.connector.connect(username='root', password='',host='localhost',database='face_recognizer',port=3306)
                mycursor = conn.cursor()
                mycursor.execute("insert into attendance_data values(%s,%s,%s,%s,%s,%s)",(
                self.var_roll.get(),
                self.var_roll.get(),
                self.var_name.get(),
                self.var_time.get(),
    ...
    #             udate = i[4]
    #             uattend = i[5]
    #             qury = "INSERT INTO attendance_data(std_id, std_roll_no, std_name, std_time, std_date, std_attendance) VALUES(%s,%s,%s,%s,%s,%s)"
